...
while (renderedPaySlip <= Integer.valueOf(GlobalVariable.leaveStartPayPeriod)) {
    if (DriverFactory.getExecutedBrowser().getName() == 'FIREFOX_DRIVER') {
        WebUI.callTestCase(findTestCase('SF/Payslip/Verify Payslip Data On Browser'), [('EmployeeID') : '', ('EmployeeName') : ''
                , ('LeaveType') : ''], FailureHandling.STOP_ON_FAILURE)
    } else {
        WebUI.click(findTestObject('Page_Print Preview/Download Button'))
